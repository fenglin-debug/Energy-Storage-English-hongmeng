...
仓库首页 README 面向使用者。下面是自行编译、检查和发布时需要的内容。

# BESS 纯血鸿蒙版

独立原生 HarmonyOS 工程，使用 ArkTS、声明式 ArkUI、Stage 模型、ArkData RelationalStore、Media Kit、AVSession、Core File Kit 和 Crypto Architecture Kit。当前发布基线：

- Bundle ID：`com.bess.salestrainer`
- 版本：`versionCode=5`、`versionName=0.4.1`
...
Android 的一份词汇语料包和两份文章包是唯一内置内容源。运行以下 Hvigor 任务会先校验锁定的 SHA-256，再复制到资源目录；两份文章包合计 22 篇：
...
按照 [SIGNING_AND_AGC.md](SIGNING_AND_AGC.md) 配置仓库外正式签名后执行：
...
签名配置、证书、私钥和 Profile 不得提交仓库。`build-profile.json5` 在仓库中应保持空的 `signingConfigs`。
...
- 0.4.1 不增加数据库表或备份字段；设置中的“自动下一篇”保存在设备本地，不进入学习备份。
